...
int extract_keys_from_header(const char* text, int* keybase, int* keyinc) {
    ...
    const char *base_ptr = strstr(text, "KEYBASE:");
    const char *inc_ptr = strstr(text, "KEYINC:");
    
    if (!base_ptr || !inc_ptr) {
        return 0;
    }
    
    if (sscanf(base_ptr, "KEYBASE:%d", keybase) != 1) {
        return 0;
    }
    if (sscanf(inc_ptr, "KEYINC:%d", keyinc) != 1) {
        return 0;
    }
...
void on_decode_button_clicked(GtkWidget *widget, gpointer data) {
    ...
    if (!input_text || *input_text == '\0') {
        show_status_message("Error: No text to decode.");
        if (input_text) g_free(input_text);
        return;
    }
    ...
    if (extract_keys_from_header(input_text, &keybase, &keyinc)) {
        ...
        auto_keys = 1;
        show_status_message("Keys automatically detected from header");
    }

    const char *keybase_str = gtk_entry_get_text(GTK_ENTRY(widgets.keybase_entry));
    const char *keyinc_str = gtk_entry_get_text(GTK_ENTRY(widgets.keyinc_entry));

    long keybase_manual = 0, keyinc_manual = 0;
    char *endptr_base, *endptr_inc;
    int keys_valid = 1;
